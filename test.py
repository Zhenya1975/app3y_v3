...
eo_list = eo_list.loc[:,
          ['level_1_description', 'eo_code', 'eo_model_name', 'eo_class_description', 'eo_description', 'teh_mesto',
           'operation_start_date', 'constr_type']]

eo_list.rename(columns={'level_1_description': 'БЕ', 'eo_code': 'EO', 'eo_model_name': 'Модель оборудования',
...
teh_karta_spisok['gruppa_str_counter'] = teh_karta_spisok['Группа'].astype(str) + "_str" + teh_karta_spisok[
    'СчтГрупТехкарт'].astype(str)

teh_karta_spisok = teh_karta_spisok.loc[:,
                   ['Тип техкарты', 'Группа', 'СчтГрупТехкарт', 'Описание', 'Гр. плановиков', 'Стратегия',
                    'Метка удаления', 'Монтажный узел', 'Статус', 'Дата изменения', 'Дата создания', 'Действит. с', 'Трудоёмкость ТК, ч',
                    'Продолжительность операций ТК, ч', 'gruppa_str_counter'
                    ]]
...
plans_toro_df['strategy_str_2'] = "'" + plans_toro_df['strategy'].astype(str)
...
update_counter_dict = {"48": 'Д2', "40": 'Д1', "41": "П1", "49": "П2"}
plans_toro_df['updated-counter'] = plans_toro_df['СчетГруппТехкарт']
plans_toro_df = plans_toro_df.replace({"updated-counter": update_counter_dict})

plans_toro_df['gruppa_str_counter'] = plans_toro_df['Группа'].astype(str) + "_str" + plans_toro_df[
    'СчетГруппТехкарт'].astype(str)
...
plans_toro_df['strategy_str_counter'] = plans_toro_df['strategy_str'].astype(str) + plans_toro_df[
    'updated-counter'].astype(str)

# print(plans_toro_df['strategy_str])

#####################  ОБЪЕДИНЯЕМ ПЛАНЫ С ТЕХАРТАМИ ПО ПОЛЮ gruppa_str_counter
teh_karta_spisok = teh_karta_spisok.loc[:, ['gruppa_str_counter', 'Трудоёмкость ТК, ч', 'Продолжительность операций ТК, ч']]
plans_toro_tehkarts = pd.merge(plans_toro_df, teh_karta_spisok, how = 'left', on = 'gruppa_str_counter')


plans_toro_df_selected = plans_toro_tehkarts.loc[:,
                         ['ПозПредупредТОРО', 'ПланПредупрТОРО', 'Позиция предупред. ТОРО - описание',
                          'Группа плановиков', 'Единица оборудования', 'Вид работы ТОРО', 'Вид заказа', 'Тип техкарты',
                          'Группа', 'СчетГруппТехкарт', 'updated-counter', 'strategy_str_counter', 'gruppa_str_counter', 'Трудоёмкость ТК, ч', 'Продолжительность операций ТК, ч']]
...
###################### ОБЪЕДИНЕНИЕ ПЛАНОВ ТОРО СО СПИСКОМ ЕО ############################
...
strategy_forms_df['strategy_str_counter'] = strategy_forms_df['strategy_str'].astype(str) + strategy_forms_df[
    'КраткНазвЦикла'].astype(str)

strategy_forms_df_selected = strategy_forms_df.loc[:,
                             ['Название стратерии', 'Единица', 'Текст цикла ПредупрТОРО', 'КраткНазвЦикла', 'Цикл',
                              'код_цикла', 'strategy_str_counter']]




##################### ОБЪЕДИНЕНИЕ ПЛАНОВ И СТРАТИЕГИЙ #############################
...
#  'Тип техкарты'
#                                                           , 'Описание', 'Гр. плановиков', 'Стратегия', 'Метка удаления',
#                                                           'Статус', 'Дата изменения', 'Дата создания', 'Действит. с', 'Название стратерии', 'Единица',
#                                                            'КраткНазвЦикла', 'Цикл', 'код_цикла', 'strategy_str',
#
#                                                           ]]
